...
export default function DosisFields({ control, register, productos, name = 'dosis' }) {
      ...
      {fields.map((field, index) => {
        ...
        return (
          <div key={field.id} className="flex items-center space-x-2 mb-2">
            <select
              {...register(`${name}.${index}.producto_id`, { required: true })}
            ...
            <input
              {...register(`${name}.${index}.cantidad`, { required: true, min: 0.01, valueAsNumber: true })}
              type="number"
              step="0.01"
